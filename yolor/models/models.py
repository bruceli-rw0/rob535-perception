...
class Darknet(nn.Module):
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        if self.training:  # train
            old_yolo_out = yolo_out
            yolo_out = list()
            for out in old_yolo_out:
                new_out = self.fc(out)
                new_out = new_out.view(*out.shape[:4], 8)
                yolo_out.append(new_out)
            ...
            for i in range(len(yolo_out)):
                yolo_out[i] = (self.fc(yolo_out[i][0]), self.fc(yolo_out[i][1]))
